...
<%@page import="daoimpl.DetalleCompraDaoImpl"%>
...
<body>
    <div class="contenedor-principal">
        ...
        <main class="contenido-principal">           
            <h1 class="titulo-contenido">Gestión de Abastecimiento</h1>

            <div class="seccion-filtros">
                <button class="btn-crear-item" id="boton-crear-producto">Registrar compra</button>
            ...
            <div class="contenedor-tabla">
                <%
                    ...
                    DetalleCompraDao detalle_compra_dao = new DetalleCompraDaoImpl();
                    List<DetalleCompra> detalle_compras = detalle_compra_dao.obtener_detalle_compras();
                    ProductoDao dao_prod = new ProductoDaoImpl();
                    CategoriaDao dao_cat = new CategoriaDaoImpl();
                    ProveedorDao dao_prov = new ProveedorDaoImpl();
                %>
                ...
                <table class="tabla-datos">
                    ...
                    <tbody>
                        <%
                        for (DetalleCompra detalle : detalle_compras) {
                            Compra compra = dao.obtener_compra_por_id(detalle.getId_compra());
                            ...
                            Proveedor prov = dao_prov.obtener_proveedor_por_id(compra.getId_proveedor());
                            Usuario usuario = dao_usuario.obtener_usuario_por_id(compra.getId_usuario());
                        %>
                        <tr>
                            <td><%= detalle.getId_compra_detalle() %></td>
                            <td><%= compra.getFecha_registro() %></td>
                            <td><%= producto.getNombre() %></td>
                            <td><%= cat.getNombre() %></td>
                            <td><%= producto.getMarca() %></td>
                            <td><%= prov.getRazon_social() %></td>
                            <td><%= detalle.getCantidad() %></td>
                            <td><%= detalle.getP_compra() %></td>
                            <td><%= detalle.getSubtotal() %></td>
                            <td><%= compra.getGuia_remision() %></td>
                            <td><%= usuario.getNombres() %></td>                          
                            <!-- ELIMINAR -->
                            <td>
                                <button class="btn-eliminar" data-id="<%= detalle.getId_compra_detalle() %>">Eliminar</button>
                            </td> 
                        </tr>
                        <% } %>
                    </tbody>
    ...
    <div id="modal-item" class="modal-superposicion">
        <div class="modal-contenido">
            ...
            <h2 id="modal-titulo">Registrar compra</h2>

            <!-- FORMULARIO PARA AGREGAR PRODUCTOS -->
            <form action="<%= request.getContextPath() %>/DetalleCompraController" method="post">
                ...
                Precio Unitario:<br>
                <input type="number" name="p_unitario" min="0" required>
                <br><br>
                ...
                <tbody>
                    <% for (DetalleCompra d : carrito) { %>
                        ...
                        <tr>
                            ...
                            <td><%= d.getCantidad() %></td>
                            <td><%= d.getP_compra() %></td>
                            <td><%= d.getCantidad() * d.getP_compra() %></td>
                        </tr>
                    <% } %>
                </tbody>
            </table>
            ...
            <form action="<%= request.getContextPath() %>/CompraController" method="post">
                <input type="hidden" name="action" id="action" value="create">
                <input type="hidden" name="id" id="id_item">

                <%
                    ProveedorDao provDao = new ProveedorDaoImpl();
                    List<Proveedor> proveedores = provDao.obtener_proveedores();
                %>
                ...
                <select name="id_proveedor" required>
                    ...
                    <% for (Proveedor prov : proveedores) { %>
                        <option value="<%= prov.getId_proveedor() %>">
                            <%= prov.getRazon_social() %>
                        </option>
                    <% } %>
                </select>
                <br><br>
                ...
                Guía de Remisión:<br>
                <input type="text" name="guia_remision"><br><br>

                <button type="submit" class="boton-guardar" >Registrar Compra</button>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        if (btnEliminarConfirmar) {
            btnEliminarConfirmar.addEventListener("click", () => {
                ...
                if (idEliminar) {
                    const url_eliminar = contextPath + '/DetalleCompraController?action=delete&id=' + encodeURIComponent(idEliminar);
